...
while (!$disparoValido) {
    ...
    // ¡FIX VITAL! Resetear las variables al principio de cada vuelta para evitar el Bucle Infinito
    $f = -1;
    $c = -1;
    $resultado = ""; 
    ...
    if (!$memoria['modo_caza']) {
        ...
        // Si no hay direcciones, recargamos y volvemos al origen
        ...
        // Si perdimos la dirección actual, sacamos una nueva del bolso
        if ($memoria['direccion_actual'] === '') {
            $memoria['direccion_actual'] = array_pop($memoria['direcciones']);
            // Siempre que probamos una dirección nueva, empezamos desde el impacto original
            $memoria['cursor_f'] = $memoria['origen_f'];
            $memoria['cursor_c'] = $memoria['origen_c'];
    }

        // Movemos el cursor hacia la dirección elegida
